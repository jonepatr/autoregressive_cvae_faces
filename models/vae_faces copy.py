...
class FaceVAE(LightningModule):
    ...
    def __init__(self, hparams):
        ...
        self.data_files = json.load(open(hparams.data_files))
        self.batch_size = hparams.batch_size
        self.bottleneck_size = hparams.bottleneck_size
        self.data_dim = hparams.data_dim
        self.group_distance_scaling = hparams.group_distance_scaling
    ...
    def __build_model(self):
        ...
        self.fc1 = nn.Linear(self.data_dim, 70)
        # self.fc2 = nn.Linear(100, 80)
        # self.fc22 = nn.Linear(80, 40)
        self.fc31 = nn.Linear(70, self.bottleneck_size)
        self.fc32 = nn.Linear(70, self.bottleneck_size)
        self.fc4 = nn.Linear(self.bottleneck_size, 70)
        # self.fc4 = nn.Linear(40, 80)
        # self.fc5 = nn.Linear(80, 100)
        # self.fc61 = nn.Linear(40, 140)
        self.fc6 = nn.Linear(70, self.data_dim)

        # self.fc1 = nn.Linear(self.data_dim, 1000)
        # self.fc2 = nn.Linear(1000, 1000)
        # self.fc22 = nn.Linear(1000, 1000)
        # self.fc31 = nn.Linear(1000, self.bottleneck_size)
        # self.fc32 = nn.Linear(1000, self.bottleneck_size)
        # self.fc40 = nn.Linear(self.bottleneck_size, 1000)
        # self.fc4 = nn.Linear(1000, 1000)
        # self.fc5 = nn.Linear(1000, 1000)
        # self.fc6 = nn.Linear(1000, self.data_dim)
    ...
    def encode(self, x_t_0, x_t_1, x_t_2):
    ...
    def vae_loss(self, output, target, mu, logvar):
        ...
        # MSE = F.mse_loss(o * weights, t * weights, reduction="sum")  # / self.data_dim
        # MSE = F.mse_loss(o, t, reduction="sum") / batch_size / self.data_dim  # / self.data_dim

        EXTRA_MSE = self.group_distance_loss(o, t) * self.group_distance_scaling
        ...
        KLD = (
            kl_annealing_factor
            # * ((self.beta * self.bottleneck_size) / self.data_dim)
            * self.beta
            * (self.kld_loss(mu, logvar))  # / batch_size / self.bottleneck_size
        )
        # KLD = kl_annealing_factor * ((self.beta * self.bottleneck_size) / batch_size) * self.kld_loss(mu, logvar)
    ...
    def visualize_pairs(self, left_output, right_output, count):
        ...
        for i, rand_int in enumerate(rand_ints):
            ...
            self.plot_lines(in_, axes[i][0])
            self.plot_lines(out, axes[i][1])
    ...
    def validation_step(self, batch, batch_nb):
        ...
        if batch_nb == 0:
            ...
            #     # rand_ints = torch.randint(x.size(0), (5,))

            #     # fig2, axes = plt.subplots(5, 2, figsize=(15, 15))
            #     # for i, rand_int in enumerate(rand_ints):
            #     #     in_ = x[rand_int].reshape(70, 2).cpu()
            #     #     out = output[rand_int].reshape(70, 2).cpu()

            #     #     self.plot_lines(in_, axes[i][0])
            #     #     self.plot_lines(out, axes[i][1])

            #     # self.experiment.add_figure("encoder/decoder", fig2)
            #     # plt.close(fig2)

            #     # Do random sampling
            #     sample = torch.randn(1, self.bottleneck_size).to(x.device)
            ...
            #     self.plot_lines(sample_res, ax1)
