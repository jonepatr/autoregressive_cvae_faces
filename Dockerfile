...
RUN apt-get update
RUN apt-get install -y libsndfile1 wget cmake build-essential tmux git ffmpeg
...
RUN pip install luigi librosa pysoundfile pytube ruffus beautifulsoup4 pandas dlib docker matplotlib pytorch_lightning pylint black torchcontrib pdbpp jupyter tensorboard
