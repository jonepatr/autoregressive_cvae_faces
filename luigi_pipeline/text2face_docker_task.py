...
class DockerTask(luigi.Task):
    ...
    def __init__(self, *args, **kwargs):
        ...
        if self.mount_tmp:
            ...
            self._host_tmp_dir = mkdtemp(
                suffix=self.task_id, prefix="luigi-docker-tmp-dir-", dir="/tmp"
            )
    ...
    def run(self):
        ...
        try:
            logger.debug(
                "Creating image: %s command: %s volumes: %s"
            ...
            if self.auto_remove:
                try:
                    ...
                    self.__logger.warning(
                        "Container " + container["Id"] + " could not be removed"
                    )
            if exit_status != 0:
                raise ContainerError(
                    container, exit_status, self.command, self._image, error
                )
            ...
            try:
                ...
                message = str(e)
            self.__logger.error(
                "Container " + container_name + " exited with non zero code: " + message
            )
            raise
